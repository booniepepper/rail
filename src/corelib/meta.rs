...
pub fn builtins() -> Vec<RailDef<'static>> {
    vec![
        RailDef::on_state("type", "Consume a value and produce the name (a string) of its type.", &[A], &[String], |quote| {
            let (thing, quote) = quote.pop();
            quote.push_string(thing.type_name())
        }),
        RailDef::on_state("defs", "Produce a list of all defined commands in the current context.", &[], &[Quote], |state| {
        ...
        RailDef::on_state("quote-all", "Go \"up\" a context, leaving the program's previous state as a quotation.", &[], &[Quote], |quote| {
